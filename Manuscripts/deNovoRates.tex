...
Next, we tried to find out if some amino acid substitutions are more probable, than the others. To this end, we calculated the gain probability ($G$) for every amino acid. Specifically for every amino acid $i$, we calculated the average probability that it substitutes any of the other nineteen amino acids, due to mutations. More precisely, the total probability that an amino acid $j$ will mutate to an amino acid $i$ is the product of the expected frequency of amino acid $j$ ($P_j$), and the substitution rate from $i$ to $j$ ($\mu_{j\to i}$). The gain probability for amino acid $i$ is thus defined as:

$$G_i = \sum_{j \neq i} P_j \times \mu_{j\to i}$$
