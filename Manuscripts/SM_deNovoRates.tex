...
\section{Effect on GC content on the probability of gene gain and loss}
...
\section{Exclusive ORF or transcription loss}
...
\caption{Exclusive ORF loss in proto-genes is more probable than exclusive transcription loss. The horizontal axis denotes the number of codons in the ORF \textbf{(A)} and GC-content \textbf{(B)}. The vertical axis shows the $\log_2$ transformed ratio of only ORF loss and only RNA loss probabilities, such that a positive value means ORF loss is more probable than RNA loss, and \textit{vice versa}.}
\end{figure}
