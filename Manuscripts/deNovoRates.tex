...
\usepackage{amsmath,amssymb,graphicx,setspace,palatino,float,subcaption, booktabs}
...
Because the mutation rate is biased (\autoref{mutbias}), the probability that a mutation does not occur in a DNA sequence depends on the composition of the sequence. 

The probability that an \texttt{A} or a \texttt{T} mutates ($\lambda_\texttt{AT}$), is described by the mutation bias (\autoref{mutbias}) as:
...
The consensus sequence of TATA-box is \texttt{TATA [T/A] A [T/A] [G/A]} \citep{tata2}. Square brackets mean that, any of the nucleotides enclosed by them, can exist in that specific position. Another study used X-ray crystallography to identify mutations that are tolerated in TATA-box consensus sequence \cite{tata1}. Thus there are eight consensus sequences, and at seven positions, a non-consensus nucleotide is tolerated (\autoref{tatatab}). This makes a TATA-box feature set that contains 136 sequences. We defined the corresponding non-feature set by generating all 8-mer DNA sequences that do not belong to the TATA-box feature set.
...
\paragraph{Inr:} The consensus sequence of Inr is \texttt{[T/G/C] [T/G/C] CA [T/G/C] [T/A]} \citep{Inr}, which gives 54 sequences in the Inr feature set. We generated the corresponding non-feature set by taking all DNA 6-mers that are distinct from the 54 Inr sequences.
...
\begin{align*}
Q_\textit{Inr-only} & = \frac{P_\textit{Inr}\times(1 - P_\textit{Inr})}{(P_\textit{TATA} + P_\textit{Inr} - P_\textit{TATA}\times P_\textit{Inr})} \\[1em]
