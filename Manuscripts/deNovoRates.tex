...
\begin{abstract}
New protein coding genes can emerge from genomic regions that previously did not contain any genes, via a process known as \textit{de novo} gene emergence. To synthesize a protein, DNA must be transcribed as well as translated, both of which need some DNA sequence features. Stable transcription requires promoters and polydenylation signal, and translation requires at least an open reading frame (ORF). We develop mathematical models based on mutation probabilities, and the assumption of neutral evolution, to find out how quickly do genes emerge and are lost. We also investigate whether different DNA features evolve in a sequential manner, and if sequence composition is biased by mutation rate. We find out that genes are lost much more rapidly than they emerge, and genes with long ORFs preferentially arise in regions that are already transcribed. Our study not only answers some fundamental question on the topic, but also provides a modeling framework for future studies.
\end{abstract}

\section{Introduction}

Organisms evolve new traits by expanding their functional genome. Evolution of new genes is one of the ways by which new traits can emerge. The definition of a gene is complicated, and has been changing constantly \citep{genepostencode}. We use the following working definition of a gene: a gene is a region of the genome that gives rise to a functional product, that is an RNA or a protein. The next big challenge is to define what a function is, and how to classify gene functions. One of the easiest definition of a gene product's function is its ability to improve the survival of the organism under one or more environmental conditions. Proteins perform the most diverse kinds of molecular functions ranging from catalysis of biochemical reactions to formation of cellular structures \citep{berg}, whereas RNAs that do not encode proteins (mRNA) or participate in protein synthesis (rRNA, tRNA), are mostly involved in regulation of gene expression \citep{fncel,Statello2021}.
...
This preference of \textit{de novo} gene emergence trajectory exists, because the rates of feature gain and the rates of feature loss are different. For example, ORFs are very likely to be lost before transcription emerges. Specifically, ORF loss is $\sim10^4$ to $\sim10^5$ times more probable than transcription gain, such that this difference increases with the ORF length. The converse is also true, that is transcription loss is more probable than ORF gain. However, this difference is not the same as that between ORF loss and transcription gain. Transcription loss is $\sim10^3$ to $\sim10^{10}$ times more likely than ORF gain, such that this difference is larger for longer ORFs. Thus, long ORFs are more likely to be lost before transcription emerges, than short ORFs. Conversely, short ORFs are more likely to stay undisrupted until the emergence of transcription.
...
Thus widespread transcription loss can suggest a negative selection of 
